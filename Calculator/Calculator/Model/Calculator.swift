...
struct Calculator: CalculatorDelegate {
	...
	func compareOperatorPriority(stackLast: String, with current: String) -> Bool {
		if ["+", "-"].contains(stackLast), ["*", "/"].contains(current)  {
			return true
		}
		return false
	}
	...
	func convertToPostfixExpression(fromInfix input: [String]) -> [String] {
		...
		for input in input {
			...
			if let currentType = try? convertToComponentType(from: input),
			   currentType == .number {
				numberString += input
			} else {
				postfix.append(numberString)
				numberString = ""
				switch compareOperatorPriority(stackLast: stack.pop() ?? "0", with: input) {
				case true:
